...
git filter-branch --force --index-filter \
  'git rm --cached --ignore-unmatch MCP/server/src/index.ts || true; \
   if [ -f MCP/server/src/index.ts ]; then \
     sed -i.bak "/AIza_REDACTED_GOOGLE_KEY/d" MCP/server/src/index.ts && \
...
git filter-branch --force --index-filter \
  'git rm --cached --ignore-unmatch MCP/server/src/index.ts || true; \
   if [ -f MCP/server/src/index.ts ]; then \
     sed -i.bak "/sk_REDACTED_ELEVENLABS_KEY/d" MCP/server/src/index.ts && \
     git add MCP/server/src/index.ts; \
   fi' \
  --tag-name-filter cat -- --all
```
...
```bash
# Create secrets.txt
echo "AIza_REDACTED_GOOGLE_KEY" > secrets.txt
echo "sk_REDACTED_ELEVENLABS_KEY" >> secrets.txt
```
